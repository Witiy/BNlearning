package BNlearning.core.learn.solver.src.obs;
...
public class ObsSearcher extends ScoreSearcher {
    ...
    public ParentSet[] obs() {
        ...
        ArrayUtils.cloneArray(voidB, forbidden);
        for (int v : vars) {
            pSet = best(v);//return the best and legal parentset which means the node's parents have not been picked
            str[v] = pSet;
            sk += pSet.sk;

            forbidden[v] = true;
      //      for(boolean t:forbidden)
        //    	System.out.print(t+" ");
        }
